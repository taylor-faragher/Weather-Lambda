...
export const getWeatherFreemium = async event => {
    let zipCode;
    ...
    try {
        ...
        const mappedData = await mapFreemiumWeatherData(oneCallData, city);
        ...
        logger.error('Error in getWeatherFreemium: ', error);
        return {
            statusCode: 500,
            body: JSON.stringify({error: 'Internal Error in Freeimum Endpoint. Please try again later.'}),
            headers: {
                'Access-Control-Allow-Origin': '*',
            },
        };
