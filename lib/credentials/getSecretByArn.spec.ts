...
describe('getSecretByArn', () => {
    it('should retrieve and decode the secret value by ARN', () => {
        const mockUnsafeUnwrap = jest.fn().mockReturnValue({ toString: jest.fn().mockReturnValue("decoded-secret-value") });
        ...
        const secretValue = getSecretByArn(mockParent, "my-secret-id", { secretArn: "arn:aws:secretsmanager:us-east-1:123456789012:secret:mySecret" });

        expect(Secret.fromSecretAttributes).toHaveBeenCalledWith(mockParent, "my-secret-id", { secretArn: "arn:aws:secretsmanager:us-east-1:123456789012:secret:mySecret" });
